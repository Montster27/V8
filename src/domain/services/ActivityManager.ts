// /Users/montysharma/Documents/v8/MMV08/src/domain/services/ActivityManager.ts

import { Activity, IActivity, ScheduledActivity } from '../models/Activity';
...
/**
 * Service that manages activities, scheduling, and completion
 * This is an elevated version of the domain model, providing additional service-level functionality
 */
export class ActivityManager {
  ...
  /**
   * Get activities scheduled for a specific time period
   * @param timeValue The time to check for activities
   * @returns Array of activities active at that time
   */
  getActivitiesAt(timeValue: TimeValue | number): ScheduledActivity[] {
    const timestamp = timeValue instanceof TimeValue ? timeValue.timestamp : timeValue;
   ...
   * @returns Array of activities that were completed
   */
  update(currentTime: TimeValue): ScheduledActivity[] {
    ...
    const activitiesToComplete = this._scheduledActivities.filter(
      sa => sa.endTime <= currentTime.timestamp && !sa.isCompleted
    );
   ...
   * Restore from state
   */
  restoreState(state: ActivityManagerState): ActivityManager {
  ...
  /**
   * Create a default ActivityManager with some predefined activities
   */
  static createDefault(): ActivityManager {
    const manager = new ActivityManager();
    
    // Add a few basic activities for testing
    manager.registerActivity({
      id: 'study_session',
      name: 'Study Session',
      description: 'Focus on academic work to improve your knowledge',
      category: 'academic',
      duration: 60, // 1 hour
      effects: [
    ...
    manager.registerActivity({
      id: 'rest_break',
      name: 'Take a Break',
      description: 'Relax and recover some energy',
      category: 'rest',
      duration: 30, // 30 minutes
      effects: [
        { resourceType: 'energy', amount: 5 },
        { resourceType: 'stress', amount: -3 }
      ],
      repeatability: 'unlimited',
      location: { type: 'dorm', name: 'Dorm Room', id: 'dorm_room' }
    });
    
    manager.registerActivity({
      id: 'social_gathering',
      name: 'Social Gathering',
      description: 'Spend time with friends to boost belonging',
      category: 'social',
      duration: 120, // 2 hours
      effects: [
        { resourceType: 'energy', amount: -5 },
        { resourceType: 'belonging', amount: 10 },
        { resourceType: 'stress', amount: -5 }
      ],
      repeatability: 'once_per_day',
      location: { type: 'campus', name: 'Student Center', id: 'student_center' }
    });
